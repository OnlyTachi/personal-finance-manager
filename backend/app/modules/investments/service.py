...
def calculate_current_lot_value(
    valor_original: float, data_original: datetime, taxa_anual_efetiva: float
) -> float:
    if taxa_anual_efetiva <= 0 or valor_original <= 0:
        return valor_original
    dias_corridos = (datetime.now() - data_original).days
    if dias_corridos <= 0:
        return valor_original
...
def update_asset_balance(db: Session, asset: models.Ativo):
    """
    Recalcula o saldo total e o IMPOSTO do ativo baseando-se em TODAS as transações.
    """
    ...
    for t in transacoes:
        if t.tipo == "Aporte":
            val_atualizado = calculate_current_lot_value(
                t.valor, t.timestamp, taxa_efetiva
            )
            lotes.append(
                {
                    "data": t.timestamp,
                    "principal_restante": t.valor,  # Quanto sobrou do dinheiro original
                    "valor_atual": val_atualizado,  # Quanto vale hoje
                }
            )
        elif t.tipo == "Saque":
            ...
            for lote in lotes:
                ...
                if valor_a_deduzir >= lote["valor_atual"]:
                    ...
                    # Regra de três para deduzir proporcionalmente do principal
    ...
    # 3. Calcula Totais e Impostos
    saldo_bruto = 0.0
    imposto_total = 0.0
    is_isento = (
        "LCI" in asset.nome.upper()
        or "LCA" in asset.nome.upper()
        or "ISENTO" in asset.nome.upper()
    )
    ...
    for lote in lotes:
        if lote["valor_atual"] > 0.01:
            ...
            if lucro > 0:
                dias = (datetime.now() - lote["data"]).days
...
# Atualizando create_transaction para garantir que chame o update sempre
def create_transaction(db: Session, transaction_in: schemas.TransacaoCreate):
    ...
    if asset:
        # Se for Renda Variável, lógica simples de soma/subtração
        if asset.tipo_indexador in ["B3", "CRYPTO", "USA"]:
            ...
            # Se for Renda Fixa, chama o recálculo completo
            db.commit()  # Commita a transação antes de recalcular
...
# ... (Manter delete_transaction, delete_asset, create_daily_snapshot, get_portfolio_history, calculate_asset_quantity como estavam no código anterior) ...
def delete_transaction(db: Session, transaction_id: str):
    ...
    if asset:
        update_asset_balance(db, asset)  # Recalcula saldo após deletar
...
def calculate_asset_quantity(db: Session, asset_id: str) -> float:
    ...
    for t in transacoes:
        if t.tipo == "Aporte":
            qtd += t.quantidade if t.quantidade else 0.0
        elif t.tipo == "Saque":
            qtd -= t.quantidade if t.quantidade else 0.0
    return max(0.0, qtd)
...
def simulate_withdrawal_fifo(
    ...
    # (Mantém a implementação anterior que já estava correta)
    # Apenas certifique-se de que calculate_current_lot_value use a taxa correta aqui também se quiser consistência
    asset = get_asset_by_id(db, asset_id)
    if not asset:
        raise Exception("Ativo não encontrado")
    ...
    for t in transacoes:
        if t.tipo == "Aporte":
            val_atual = calculate_current_lot_value(t.valor, t.timestamp, taxa_efetiva)
            lotes.append(
                {
                    "id": t.id,
                    "data": t.timestamp,
                    "principal_inicial": t.valor,
    ...
    for lote in lotes:
        ...
        detalhes.append(
            f"Lote {lote['data'].strftime('%d/%m/%Y')}: Sacado R$ {qtd_sacada:.2f} (Lucro R$ {lucro:.2f})"
        )
...
def refresh_all_assets_prices(db: Session):
    # (Mesma lógica do passo anterior, apenas garante que usa update_asset_balance para RF)
    assets = db.query(models.Ativo).all()
    updated_count = 0
    for asset in assets:
        if asset.tipo_indexador in ["B3", "CRYPTO", "USA"] and asset.ticker:
            preco_atual = price_service.get_price(asset.ticker, asset.tipo_indexador)
            if preco_atual > 0:
                qtd_total = calculate_asset_quantity(db, asset.id)
                novo_valor_bruto = qtd_total * preco_atual
                asset.valor_atual_bruto = round(novo_valor_bruto, 2)
                asset.valor_liquido_estimado = round(novo_valor_bruto, 2)
                updated_count += 1
                db.add(asset)
        elif asset.tipo_indexador in ["CDI", "PRE", "IPCA", "MANUAL"]:
            update_asset_balance(db, asset)
            updated_count += 1
    db.commit()
    return {"message": f"Preços atualizados: {updated_count}"}
